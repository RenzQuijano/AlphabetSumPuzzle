    ...
    public static void main(String[] args){
        ...
        while(keyboard.hasNextLine()){
            ...
            for(int i = 0; i < tokens.length-1;i++)
                if(tokens[i].length() != 0)//e.g. operands = [THE, BEST, SPOT]
...
//Your code here!
        ...
        for(char variable: variables)
            if(digitValues[variable] == -1){//varible hasn't been assigned to any value yet!
                        complete = false;//assinment is not complete
...
//for extra credits, you should also consider more conditions before moving forward with printing the solutions
            ...
            for(int i = 0; i < 10;i++) {
                digitValues[unassignedVariable] = i;//assign the unassignedVarialbe to one of the possible digits 0,1,..,9
